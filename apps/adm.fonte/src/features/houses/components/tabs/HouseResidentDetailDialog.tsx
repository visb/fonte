...
interface HouseResidentDetailDialogProps {
  residentId: string | null;
  onClose: () => void;
}

/** Dialog autossuficiente com o resumo do filho + atalho para a página completa. */
export function HouseResidentDetailDialog({ residentId, onClose }: HouseResidentDetailDialogProps) {
  const navigate = useNavigate();
  const { data: detail } = useResidentById(residentId ?? '');

  return (
    <Dialog open={!!residentId} onOpenChange={(open) => !open && onClose()}>
        ...
        {!detail ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end pt-2">
              <Button variant="outline" size="sm" onClick={() => { onClose(); navigate(`/residents/${detail.id}`); }}>
                Ver página completa
              </Button>
            </div>
          </div>
