...
export function ResetResidentPasswordDialog({ open, onClose, resident }: Props) {
  const [password] = useState(() => generatePassword());
  const [showPassword, setShowPassword] = useState(false);
  const [copied, setCopied] = useState(false);

  const mutation = useResetResidentPassword(resident?.id ?? '');
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Uma nova senha será gerada para <strong>{resident?.name}</strong>. O interno deverá alterá-la no próximo acesso.
          </DialogDescription>
        </DialogHeader>
