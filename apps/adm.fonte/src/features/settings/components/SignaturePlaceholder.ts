...
// editável internamente — o texto por trás continua sendo o token, selecionável
// e apagável normalmente.
function buildPlaceholderBox(): HTMLElement {
...
export function buildSignatureDecorations(doc: ProseMirrorNode): DecorationSet {
  ...
  doc.descendants((node, pos) => {
    ...
    for (const [from, to] of findSignatureRanges(node.text)) {
      ...
      decorations.push(
        Decoration.widget(start, buildPlaceholderBox, {
          side: 1,
          key: `signature-${start}`,
        }),
      );
  ...
  addProseMirrorPlugins() {
    return [
      new Plugin<DecorationSet>({
        ...
        state: {
          init: (_config, state) => buildSignatureDecorations(state.doc),
          ...
          apply: (tr, old) =>
            tr.docChanged ? buildSignatureDecorations(tr.doc) : old,
        },
        props: {
          decorations(state) {
