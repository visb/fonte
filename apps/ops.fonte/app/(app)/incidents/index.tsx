    ...
    <View className="flex-1 bg-gray-50">
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: { id: string; date: string; severity: string; description: string; responsible: { name: string }; resident: { name: string } | null } }) => (
            <View className="bg-white rounded-xl border border-gray-100 px-4 py-3">
              <View className="flex-row items-center justify-between mb-1">
                ...
                <View
                  ...
                  style={{ backgroundColor: `${SEVERITY_COLOR[item.severity]}20` }}
                >
                  <Text className="text-xs font-medium" style={{ color: SEVERITY_COLOR[item.severity] }}>
