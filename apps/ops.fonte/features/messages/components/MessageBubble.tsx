...
export function MessageBubble({ message, isOwn }: Props) {
  ...
  return (
    <View className={`mb-2 max-w-[75%] ${isOwn ? 'self-end' : 'self-start'}`}>
      ...
      <View className={`rounded-2xl overflow-hidden ${isOwn ? 'bg-[#272950]' : 'bg-white border border-gray-100'}`}>
        {message.attachmentType === 'image' && resolvedUrl ? (
          ...
          >
            ...
            <View className="flex-1">
              <Text className={`text-sm font-medium ${isOwn ? 'text-indigo-200' : 'text-indigo-700'}`} numberOfLines={1}>
