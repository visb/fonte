...
describe('StaffController', () => {
  it('getMe delegates to findByUserId', async () => {
    const s = svc();
    await new StaffController(s as never).getMe(user);
    expect(s.findByUserId).toHaveBeenCalledWith('u1');
  });

  it('updateMe delegates with dto', async () => {
    const s = svc();
    await new StaffController(s as never).updateMe(user, { name: 'x' } as never);
    expect(s.updateMe).toHaveBeenCalledWith('u1', { name: 'x' });
  });

  it('uploadPhotoMe rejects a missing file', () => {
    expect(() => new StaffController(svc() as never).uploadPhotoMe(user, undefined)).toThrow(BadRequestException);
  });

  it('uploadPhotoMe rejects a non-image', () => {
    expect(() =>
      new StaffController(svc() as never).uploadPhotoMe(user, { mimetype: 'text/plain', size: 1 } as Express.Multer.File),
    ).toThrow(BadRequestException);
  });

  it('uploadPhotoMe rejects a file over 5 MB', () => {
    expect(() =>
      new StaffController(svc() as never).uploadPhotoMe(user, { mimetype: 'image/png', size: 6 * 1024 * 1024 } as Express.Multer.File),
    ).toThrow(BadRequestException);
  });

  it('uploadPhotoMe accepts a valid image', async () => {
    const s = svc();
    await new StaffController(s as never).uploadPhotoMe(user, img);
    expect(s.uploadPhotoMe).toHaveBeenCalledWith('u1', img);
  });

  it('findAll passes the caller scope', async () => {
    const s = svc();
    await new StaffController(s as never).findAll(user);
    expect(s.findAll).toHaveBeenCalledWith({ role: 'ADMIN', userId: 'u1' });
  });

  it('findOne / create / update / remove delegate', async () => {
    const s = svc();
    const c = new StaffController(s as never);
    await c.findOne('s1');
