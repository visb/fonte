...
@Injectable()
export class SupplyRoomService {
  constructor(
    @InjectRepository(SupplyRoomItem)
    private itemRepo: Repository<SupplyRoomItem>,
    @InjectRepository(SupplyRoomMovement)
    private movementRepo: Repository<SupplyRoomMovement>,
  ) {}

  findItems(houseId?: string): Promise<SupplyRoomItem[]> {
  ...
  async findItem(id: string): Promise<SupplyRoomItem> {
    const item = await this.itemRepo.findOne({ where: { id } });
    if (!item) throw new NotFoundException(`Supply room item ${id} not found`);
    return item;
  }

  createItem(dto: CreateItemDto): Promise<SupplyRoomItem> {
    return this.itemRepo.save(this.itemRepo.create(dto));
  }

  async updateItem(id: string, dto: UpdateItemDto): Promise<SupplyRoomItem> {
  ...
  findMovements(houseId?: string, itemId?: string): Promise<SupplyRoomMovement[]> {
  ...
  async createMovement(dto: CreateMovementDto): Promise<SupplyRoomMovement> {
