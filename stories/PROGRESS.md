...
| 11 | 53 — testes ops.fonte (jest-expo + Playwright web) | [OK] | test:ops:unit 21✓ (jest-expo) + tsc limpo + e2e web Playwright 4✓ | 6e884e8 | 7f767c9 |
| 12 | 54 — testes app.fonte (jest-expo + Playwright web) | [OK] | test:app:unit 20✓ (jest-expo) + tsc limpo + e2e web Playwright 4✓ | 1bf3a02 | b24dddf |
...
[OK] 54 — testes: pnpm test:app:unit 20 (jest-expo) + tsc limpo + e2e web Playwright 4 — commit: 1bf3a02 — merge: b24dddf — 2026-06-17 — Maestro nativo opcional (não-gate). ESPELHOU a story 53 (ops.fonte) 1:1 p/ o app.fonte (portal do familiar, role RELATIVE — mesmas majors: Expo SDK 52, jest@~29.7 + jest-expo@~52.0.6 + @testing-library/react-native@^12.9 + @types/jest + @playwright/test@^1.60). jest.config.js (preset jest-expo, transformIgnorePatterns padrão estendido c/ nativewind/@fonte, moduleNameMapper @/→<rootDir>, testPathIgnorePatterns p/ e2e/ Maestro + e2e-web/ Playwright + dist/.expo), jest.setup.js (@testing-library/react-native/extend-expect + MOCK de @expo/vector-icons via Proxy → Text c/ accessibilityLabel: necessário porque sob jest-expo o expo-font quebra ao renderar ícones — "loadedNativeFonts.forEach is not a function"; ops não esbarrou nisso porque seu CategoryBadge não tem ícone), jest-matchers.d.ts (importa extend-expect p/ o tsc enxergar toBeOnTheScreen etc.); scripts app test:unit=jest / test:e2e=playwright test + build:web:test; raiz test:app:unit/test:app:e2e (reservados pela 49) já apontavam. Unit (4 suites, ao lado do código, 20 testes): lib/errors.test.ts (getErrorMessage string/array→join/fallback padrão+custom/null; getApiErrorCode extrai code/undefined), lib/queryKeys.test.ts (relativeMe/consents estáticas, messages.thread/directThread/houseStaffThreads, wishlist.byResident, não-colisão), features/messages/hooks/useMessages.test.tsx (QueryClientProvider + jest.mock('@/lib/api'): useThread repassa residentId/relativeId e enabled=false c/ ids vazios; useSendMessage envia texto com resident/relative na mutation e ignora conteúdo só-espaços), features/wishlist/components/WishlistItemCard.test.tsx (render RTL native: descrição, mostra Quantidade quando >1, oculta quando =1). e2e web Playwright (export web FUNCIONOU): playwright.config.ts espelhando ops (chromium, testDir e2e-web/, baseURL :8084 — porta própria p/ não colidir c/ ops :8083, webServer `npx serve dist -s`); build:web:test bakeia EXPO_PUBLIC_API_URL=localhost:3001 com --clear (mesmo truque da 53 — sem --clear o Metro reaproveita cache c/ a URL :3000 padrão e o login falha; documentado no spec); specs e2e-web/ (login.spec: login válido→home "Maria Testadora", senha errada→"E-mail ou senha incorretos."; home.spec: dados do acolhido/casa João Testador/Casa Teste/Rua das Flores,123 + logout→"Portal do familiar") contra API teste :3001 seed familiar@fonte.com/familiar123. Telas só-nativo (checkin QR/câmera expo-camera, foto/doc ImagePicker, áudio expo-av, push) e o first-login set-password ficam só no Maestro — documentado nos specs. Maestro e2e/*.yaml (11) INTACTOS como e2e nativo opcional. .gitignore ganhou /dist /.expo /test-results /playwright-report /blob-report /playwright/.cache. Sem skip/only/xfail.

[OK] 53 — testes: pnpm test:ops:unit 21 (jest-expo) + tsc limpo + e2e web Playwright 4 — commit: 6e884e8 — merge: 7f767c9 — 2026-06-17 — Maestro nativo opcional (não-gate). IMPLEMENTADO: tooling unit jest-expo p/ Expo SDK 52 — jest@~29.7 + jest-expo@~52.0.6 + @testing-library/react-native@^12.9 + @types/jest; jest.config.js (preset jest-expo, transformIgnorePatterns padrão estendido c/ nativewind/@fonte, moduleNameMapper @/→<rootDir>, testPathIgnorePatterns p/ e2e/ Maestro + e2e-web/ Playwright + dist/.expo), jest.setup.js (@testing-library/react-native/extend-expect), jest-matchers.d.ts (importa extend-expect p/ o tsc enxergar toBeOnTheScreen etc.); scripts ops test:unit=jest / test:e2e=playwright test; raiz test:ops:unit/test:ops:e2e (reservados pela 49) já apontavam. Unit (3 suites, ao lado do código): lib/inventoryUtils.test.ts (toNumber finito/null/NaN, formatQuantity vírgula+arredonda 2 casas, formatDateBR ISO→dd/mm/aaaa + timestamp + fallback, toISODate zero-pad, movementLabel IN/OUT), features/supply-room/hooks/useSupplyRoom.test.tsx (QueryClientProvider + vi-equivalente jest.mock('@/lib/api'): useSupplyRoomItems repassa {houseId} e enabled=false c/ houseId null; useCreateSupplyItem repassa payload na mutation), features/supply-room/components/CategoryBadge.test.tsx (render RTL native checa rótulo PT CLEANING/PPE/OTHER). e2e web Playwright (export web FUNCIONOU): playwright.config.ts espelhando o adm (chromium, testDir e2e-web/, baseURL :8083, webServer `npx serve dist -s`); build:web:test bakeia EXPO_PUBLIC_API_URL=localhost:3001 com --clear (sem --clear o Metro reaproveita cache c/ a URL :3000 padrão e o login falha — diagnosticado e documentado no spec); specs e2e-web/ (login.spec: login válido→dashboard "Bem-vindo,"/"Operador Teste", senha errada→erro; dashboard.spec: ações rápidas Ministérios/Almoxarifado/Atividades + logout→tela login) contra API teste :3001 seed operator@fonte.com/operator123. Maestro e2e/*.yaml (10) INTACTOS como e2e nativo opcional. .gitignore ganhou /dist /.expo /test-results /playwright-report. Sem skip/only/xfail.
